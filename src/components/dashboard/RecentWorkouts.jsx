      ...
      {workouts.length > 0 ? (
        <div className="space-y-3">
          {workouts.map(workout => {
            const workoutName = workout.workoutType === 'circuit' ? 'Circuit Training' : workout.name;

            return (
              <div key={workout.docId} className="relative rounded-lg bg-gray-800 p-3 shadow-sm">
                <div className="cursor-pointer" onClick={() => onSelect(workout)}>
                  <div className="flex items-center justify-between">
                    <p className="font-bold text-cyan-400">{workoutName}</p>
                    <div className="ml-2 flex items-center gap-2 text-sm">
                      {workout.containsNewPR && <span title="New Personal Record!">🥇</span>}
                      {workout.workoutType === 'circuit' && (
                        <>
                          <span title="Time Badge"><BadgeIcon badge={workout.timeBadge} /></span>
                          <span title="Completion Badge"><BadgeIcon badge={workout.exerciseBadge} /></span>
                        </>
                      )}
                    </div>
                  </div>
